...
// formula between the two kids). Replaced 2026-09-06 with genuinely
// separate per-kid goals -- "this kid's will go to this kid" -- so
// surplus can be directed to a specific kid's 529.
//
// The stored `goal` string is a STABLE key ("Education funding -
// kid_<id>", built from that kid's own database id), NOT the kid's
// configurable display name -- a rename in Settings/Kids must never
// orphan an existing surplus_allocations row. Originally two fixed keys
// ("Education funding - Abby"/"Cooper"); generalized (kids-variable-
// count, 2026-09-09) to one key per row currently in the `kids` table,
// via useKids() -- a household with 0 kids sees only the fixed goals
// below; up to 5 kids each get their own row.
const FIXED_GOALS_BEFORE = ['Emergency reserve', 'High-interest debt payoff', 'Retirement contributions']
const FIXED_GOALS_AFTER  = ['Tax reserve', 'Taxable investing', 'Other goal']
const educationGoalKey = kidId => `Education funding - kid_${kidId}`

export default function SurplusPlan(){
  const[d,setD]=useState(null);const[values,setValues]=useState({})
  const { kids } = useKids()
  const load=()=>{
  ...
  const GOALS = [...FIXED_GOALS_BEFORE, ...kids.map(k => educationGoalKey(k.id)), ...FIXED_GOALS_AFTER]
  const label = g => {
    const kid = kids.find(k => g === educationGoalKey(k.id))
    return kid ? `Education funding — ${kid.name}` : g
  }
