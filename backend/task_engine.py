...
CURRENT_YEAR = datetime.now().year  # snapshot at import time -- kept only for
# tests to reference "the current year" without importing datetime; nothing
# in this module computes with it (see generate_tasks' own current_year).
...
def generate_tasks(accounts: List[Dict], inputs: Dict, projections: Dict, education: Dict, policy: Dict = None) -> List[Dict]:
    tasks = []
    # Computed fresh per call, not from the module-level CURRENT_YEAR (which
    # is fixed at import time) — this app runs as a long-lived process
    # (start.sh), so a stale module constant would keep generating every
    # annual task under the previous year's auto_key indefinitely after a
    # Dec 31 -> Jan 1 rollover with no restart (audit finding, 2026-09-14, P2).
    ...
    # Reads the household's actual saved investment_policies target rather
    # than a hardcoded percentage split -- this used to disagree with
    # report_generator.py's own separately hardcoded split, and with
    # whatever the household had really configured (audit finding,
    # 2026-09-14, P2).
    from holdings_engine import policy_coarse_summary
    allocation_summary = policy_coarse_summary(policy)
    if allocation_summary:
        target_desc = (f"{allocation_summary['stock_pct']}% stock / {allocation_summary['bonds_pct']}% bonds / "
                        f"{allocation_summary['real_estate_pct']}% real estate")
    else:
        target_desc = "the target set on the Portfolio Setup page"
    task("investments", "Review asset allocation",
         f"Check that current allocation matches target ({target_desc}).",
...
def sync_auto_tasks(conn, accounts, inputs, projections, education, policy=None):
    """
    ...
    generated = generate_tasks(accounts, inputs, projections, education, policy=policy)
    inserted  = 0
    for t in generated:
